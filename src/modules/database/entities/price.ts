...
export class Price extends TypedData {
     ...
     */
    public async selectLast2PricesOnEachProduct(driver: IDbDriver): Promise<Price[]> {
        return await driver.withSession(async (session) => {
    ...
    public static async getByDate(driver: IDbDriver, data: { from: Date; to: Date }): Promise<Price[]> {
        return await driver.withSession(async (session) => {
            const query = `
