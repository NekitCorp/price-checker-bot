...
export class Product extends TypedData {
    ...
    public static async getProductsWithSubscription(driver: IDbDriver): Promise<Product[]> {
        return await driver.withSession(async (session) => {
            const query = `
    ...
    public static async get(driver: IDbDriver, data: Pick<IProduct, 'id'>): Promise<Product | null> {
        return await driver.withSession(async (session) => {
            const query = `
